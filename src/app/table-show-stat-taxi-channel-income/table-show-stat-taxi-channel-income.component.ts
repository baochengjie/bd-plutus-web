...
@Component({
  selector: 'app-table-show-stat-taxi-channel-income',
  templateUrl: './table-show-stat-taxi-channel-income.component.html',
  styleUrls: ['./table-show-stat-taxi-channel-income.component.css']
})
export class TableShowStatTaxiChannelIncomeComponent implements OnInit {
  dataUrl_param = 'http://localhost:8080/stat_taxi_finance_pay_channel_param';
  dataUrl = 'http://localhost:8080/stat_taxi_finance_pay_channel';
  dataSet: Array<any> = [];
  _allChecked = false;
  ...
  ngOnInit() {
    this.http.get('http://localhost:8080/stat_taxi_finance_pay_channel').subscribe(res => {
